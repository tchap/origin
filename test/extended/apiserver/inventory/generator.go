...
// FormatGoCode formats the inventory as Go code for the generated file.
func FormatGoCode(entries []ServedAPIEntry, kubeMinorVersion int) (string, error) {
	...
	fmt.Fprintf(&b, "import utilversion \"k8s.io/apimachinery/pkg/util/version\"\n\n")
	...
	fmt.Fprintf(&b, "// ForKubeVersion returns the Kubernetes API inventory for the given version.\n")
	fmt.Fprintf(&b, "// Returns found=false if the version is not in the generated inventory.\n")
	fmt.Fprintf(&b, "func ForKubeVersion(v *utilversion.Version) ([]ServedAPIEntry, bool) {\n")
	fmt.Fprintf(&b, "\tif v.Major() != 1 {\n")
	fmt.Fprintf(&b, "\t\treturn nil, false\n")
	fmt.Fprintf(&b, "\t}\n")
	fmt.Fprintf(&b, "\tswitch v.Minor() {\n")
	fmt.Fprintf(&b, "\tcase %d:\n", kubeMinorVersion)
	fmt.Fprintf(&b, "\t\treturn kubeAPIs1%d, true\n", kubeMinorVersion)
